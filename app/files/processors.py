...
async def read_excel(path: str) -> dict:
    try:
        ...
        for name in wb.sheetnames:
            ws = wb[name]; sheets[name] = [[c.value for c in row] for row in ws.iter_rows(max_row=50)]
        return {"type":"excel","sheets": sheets}
    except Exception as e: return {"error": str(e)}

async def read_json(path: str) -> dict:
    try: with open(path) as f: return {"type":"json","data": json.load(f)}
    except Exception as e: return {"error": str(e)}

async def read_csv(path: str) -> dict:
    try:
...
FILE_HANDLERS = {".pdf": read_pdf, ".docx": read_docx, ".xlsx": read_excel, ".json": read_json, ".csv": read_csv}
